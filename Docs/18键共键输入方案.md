...
创建 `lua/sharedkey_shuangpin_precise_input_processor.lua`，拦截大写字母输入：
...
创建 `lua/sharedkey_shuangpin_precise_input_filter.lua`，使用反查获取候选的真实拼音并过滤。
...
精确输入过滤器通过 `ReverseLookup` 获取候选字的真实双拼和辅助码。多音字会返回空格分隔的多组编码，例如：
...
旧实现只解析第一组编码。因此使用滑动精确输入第二或后续读音时，候选可能被误判为不匹配，表现为部分汉字偶发缺失。

### 修复

v9 Filter 将全部反查编码解析并缓存为数组。音码、形码和精确位置只要与其中任意一组编码同时匹配，就保留该候选。
...
修复日期为 2026-07-19。由于原候选缺失问题为偶发问题，单次测试无法确认彻底修复，当前状态为已部署、需长期观察。
...
| `lua/sharedkey_shuangpin_precise_input_processor.lua` | 拦截大写字母，记录精确输入位置（字符级别） |
| `lua/sharedkey_shuangpin_precise_input_filter.lua` | 使用 ReverseLookup 的全部编码过滤模糊候选（v9） |
...
Lua 脚本的核心逻辑（Processor 和 Filter）不依赖具体的共键布局，只依赖 `fuzzy_pairs` 配置表。因此，要支持不同的共键布局，只需修改配置即可。

### v5 版本：从配置文件读取模糊对

v5 版本的 Filter 支持从 YAML 配置文件读取 `fuzzy_pairs`，无需修改 Lua 代码。

**配置方式**：在 `moqi_xh-trime.custom.yaml` 中添加：

```yaml
# 精确输入配置
precise_input:
  # 模糊对列表，每个元素是两个共键字母
  # 格式：["ab", "cd", ...] 表示 a-b 共键、c-d 共键
  fuzzy_pairs:
    - "we"  # WE 共键
    - "rt"  # RT 共键
    - "io"  # IO 共键
...
如果没有配置 `precise_input/fuzzy_pairs`，则使用默认的18键布局。
...
```yaml
# moqi_xh-trime-14key.custom.yaml
precise_input:
  fuzzy_pairs:
    - "qw"  # QW 共键
    - "er"  # ER 共键
...
需要新建 `shouxin_14key.trime.yaml`，配置14键的滑动行为。

### 其他共键布局

对于任意共键布局，只需要：

1. **配置 `precise_input/fuzzy_pairs`**：列出所有共键对
2. **配置 `speller/algebra`**：添加对应的 `derive` 模糊规则
3. **配置键盘布局**：设置共键的滑动发送大写字母

Lua 脚本（Processor 和 Filter）无需修改，自动适配新布局。
...
| v5.0 | 2026-01-03 | 支持从配置文件读取模糊对，兼容14键等其他共键布局 |
| v6.0 | 2026-07-19 | Processor v3、Filter v10：修复状态残留、多音节候选区间、精确整句重组、多音字反查和完整候选流过滤 |
