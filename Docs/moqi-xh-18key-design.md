...
- [ ] 4.4 测试多字连续输入
- [ ] 4.5 修复发现的问题
- [ ] 4.6 修复候选下拉收起后退格自动重新展开
    - 输入多字编码，例如“你好”，打开 Trime 候选下拉区，此时扩大的候选区遮盖键盘
    - 在下拉区选择第一个字“你”，保留后续 `h(xc)` 处于候选状态，然后收起候选区
    - 点击退格会正常删除一次，但候选下拉区同时自动重新展开；后续每次退格都会重复展开
    - 未打开过候选下拉区时无法复现，初步判断与 Trime 3.3.11 的候选面板展开状态或刷新逻辑有关
    - 当前状态：待定位，排在现有任务列表最后
